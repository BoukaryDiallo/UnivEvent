...
class ResultatController extends Controller
{
    ...
    public function classement(Evenement $evenement)
    {
        ...
        return Resultat::with('utilisateur:id,name,email,role')
    ...
    public function exportPdf(Request $request, Evenement $evenement)
    {
        ...
        $resultats = Resultat::with('utilisateur:id,name,email,role')
            ->where('evenement_id', $evenement->id)
            ->orderBy('classement')
            ->orderByDesc('note')
            ->get();
    ...
    {
        $resultat = Resultat::with('utilisateur')->where('evenement_id', $evenement->id)->where('utilisateur_id', $userId)->first();
