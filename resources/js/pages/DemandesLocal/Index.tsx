...
interface Props {
  demandes: DemandeLocal[]
}

export default function DemandesLocalIndex({ demandes }: Props) {
  const { put, processing, setData } = useForm({ commentaire: '' })

  const handleValider = (id: number) => {
    put(`/demandes-local/${id}/valider`)
  }
    ...
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-8 bg-gradient-to-br from-slate-50 to-slate-100">
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          <h1 className="text-3xl font-semibold text-slate-800 tracking-tight">Demandes de locaux</h1>
          <p className="text-slate-500 mt-2 text-base font-light">Gérez les demandes de réservation de salles</p>
        </div>
          ...
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-6 py-3.5 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Salle</th>
            ...
            <tbody className="bg-white divide-y divide-slate-200">
              {demandes.map((demande) => (
                <tr key={demande.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-slate-600">{demande.salle_souhaitee}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-slate-600">
                    {new Date(demande.date).toLocaleDateString('fr-FR')} à {new Date(demande.date).toLocaleTimeString('fr-FR')}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
              ...
              {demandes.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                    ...
                    <p className="font-medium">Aucune demande de local</p>
                  </td>
                </tr>
              )}
            </tbody>
