    ...
    return (
        <div className="rounded-[2rem] border border-slate-200 bg-white/90 p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950/70">
            ...
            <div className="mt-5 space-y-3">
                {items.length ? (
                    items.map((item) => (
                        <Link key={item.id} href={`/module5/events/${item.event_id}`} className="flex gap-3 rounded-2xl border border-slate-200 px-4 py-3 transition hover:border-sky-300 hover:bg-sky-50 dark:border-slate-800 dark:hover:bg-slate-900">
